...
const PostWidget = ({
  ...
  const deletePost = async (id) => {
    try {
      const response = await fetch(`http://localhost:3001/business/${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
  ...
  const patchLike = async () => {
    const response = await fetch(`http://localhost:3001/business/${postId}/like`, {
      method: "PATCH",
      headers: {
  ...
  const updatePost = async (postId, updatedData) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/business/${postId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
